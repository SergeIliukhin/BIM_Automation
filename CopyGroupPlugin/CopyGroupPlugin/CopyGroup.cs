    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Room sourceRoom = GetRoomByPoint(doc, point);
                XYZ sourceRoomCenter = GetElementCenter(sourceRoom);
                XYZ currentRoomOffset = sourceRoomCenter + offset;


                Transaction transaction = new Transaction(doc);
                transaction.Start("Копирование группы");
                doc.Create.PlaceGroup(currentRoomOffset, group.GroupType);
                transaction.Commit();
            }
